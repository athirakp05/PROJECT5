        ...
        if user is not None:
            if user.is_active:  # Check if the user is active
                ...
                if user.role == 'Admin':
                    ...
                    if user.deliveryboy.is_approved:
                        messages.success(request, "Login successful!")
...
def admin_delivery_view(request):
    ...
    return render(request, 'admin/admin_delivery_view.html', {'delivery_boys': delivery_boys})

@login_required
def admin_approve_delivery(request, delivery_boy_id):
    ...
    if request.method == 'POST':
        ...
        if action == 'approve':
            ...
            delivery_boy.save()
            messages.success(request, f'{delivery_boy.first_name} {delivery_boy.last_name} has been approved.')
        elif action == 'reject':
            delivery_boy.delete()
            messages.warning(request, f'{delivery_boy.first_name} {delivery_boy.last_name} has been rejected.')
        return redirect('admin_delivery_view')
    
@login_required
def del_edit_profile(request):
    user = request.user
    delivery_profile, created = DeliveryBoyEditProfile.objects.get_or_create(user=user.deliveryboy.user)
    if request.method == 'POST':
        form = DeliveryBoyEditProfileForm(request.POST, request.FILES, instance=delivery_profile)
        if form.is_valid():
            form.save()
            return redirect('delivery_profile')  
    else:
        form = DeliveryBoyEditProfileForm(instance=delivery_profile)
    return render(request, 'del/del_edit_profile.html', {'form': form})
@login_required
